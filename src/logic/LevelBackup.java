...
class LevelBackup {
    ...
    LevelBackup(int rowLength, int colLength) {
    ...
    LevelBackup(Cell[][] cells, String levelName) {
    ...
    public void setLevelName(String levelName) {
        if (levelName == null)
            throw new IllegalArgumentException("LevelBackup name must not be null!");
        if (levelName.length() == 0)
            throw new IllegalArgumentException("LevelBackup name must not be empty string!");
    ...
    void putField(int x, int y, int fieldValue) {
        ...
        cells[x][y].setToFieldType(fieldValue);
    }
    void putField(Coord c, int fieldValue) {
        putField(c.getW(), c.getH(), fieldValue);
    }
    ...
    Cell.Type getTypeOf(Coord c) {
        return getTypeOf(c.getW(), c.getH());
    }
    ...
    private Coord getNewCoords(Coord coord, Directions dir) {
        ...
        int newX = coord.getW() + modX;
        int newY = coord.getH() + modY;
        return new Coord(newX, newY);
    }

    private boolean moveToFrom(Coord to, Coord from) {
        Cell fromCell = cells[from.getW()][from.getH()];
        Cell toCell = cells[to.getW()][to.getH()];
...
/*
        ...
        Cell.Type type = getTypeOf(newCoord.getW(), newCoord.getH());
        if (type == Cell.Type.FIELD || type == Cell.Type.EMPTY) {
            cells[playerCoord.getW()][playerCoord.getH()].setType(Cell.Type.EMPTY);
            cells[newCoord.getW()][newCoord.getH()].setType(Cell.Type.PLAYER);
            playerCoord = newCoord;
        }

        if (type == Cell.Type.BOX || type == Cell.Type.MARKED_BOX) {
            Coord boxCoord = getNewCoords(newCoord, dir);
            Cell.Type nextType = getTypeOf(boxCoord.getW(), boxCoord.getH());
            if (nextType == Cell.Type.FIELD || nextType == Cell.Type.EMPTY) {
                ...
                playerCoord = newCoord;
                // move box
                cells[boxCoord.getW()][boxCoord.getH()].setType(Cell.Type.BOX);
            }
            if (nextType == Cell.Type.BSPACE) {
                ...
                playerCoord = newCoord;
                // move box
                cells[boxCoord.getW()][boxCoord.getH()].setType(Cell.Type.BOX_ON_BSPACE);
            }
